...
def generate_random_arrival_time():
    start_time = 6 * 3600  # 6 AM
    end_time = 24 * 3600  # 12 AM
...
def pick(passenger_id, arrival_time, src, dest, path):
    ...
    conn.commit()
    cursor.execute("UPDATE passenger SET departure_time = ?, flag = 1, pod_departure = ?, travel_time = ?, waiting_time = ? WHERE pid = ?",
                   (final_time, pod_departure, travel_time, waiting_time, passenger_id))
    conn.commit()
...
# Process each passenger count individually
for passenger_count, output_file in zip(passenger_counts, output_files):
    ...
    for _ in range(passenger_count):
        ...
        cursor.execute("""
            INSERT INTO passenger (arrival_stop, destination_stop, arrival_time, departure_time, flag, pod_departure, from_stop, path, travel_time, waiting_time)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (arrival_stop, destination_stop, arrival_time, 0, 0, 0, None, path, 0, 0))
    conn.commit()
    ...
    # Export to CSV with additional columns
    cursor.execute("SELECT * FROM passenger")
    rows = cursor.fetchall()
    with open(output_file, 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow([
            'pid', 'arrival_stop', 'destination_stop', 'arrival_time',
            'departure_time', 'flag', 'pod_departure', 'from_stop', 'path',
            'travel_time', 'waiting_time', 'destination_time', 'distance'
        ])
        for row in rows:
