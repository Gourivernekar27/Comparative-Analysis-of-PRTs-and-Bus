...
def pick(passenger_id, arrival_time, src, dest, path):
    ...
    conn.commit()
    cursor.execute("UPDATE passenger SET departure_time = ?, flag = 1, pod_departure = ? WHERE pid = ?",
                   (final_time, pod_departure, passenger_id))
    cursor.execute("UPDATE passenger SET waiting_time = ? WHERE pid = ?",
                   (pod_departure - arrival_time, passenger_id))
    conn.commit()
...
for passenger_count, output_file in zip(passenger_counts, output_files):
    ...
    # Generate passengers for this set
    ...
    with open(output_file, 'w', newline='') as csvfile:
        ...
        csvwriter.writerow(
            ['pid', 'arrival_stop', 'destination_stop', 'arrival_time', 'departure_time', 'flag', 'pod_departure',
             'from_stop', 'path', 'travel_time', 'waiting_time', 'destination_time', 'distance'])
        for row in rows:
