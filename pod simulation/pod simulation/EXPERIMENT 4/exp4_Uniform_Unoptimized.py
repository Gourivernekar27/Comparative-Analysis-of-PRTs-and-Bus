...
# Initialize distance-based graph
...
def pick(passenger_id, arrival_time, src, dest, path):
    ...
    cursor.execute("UPDATE passenger SET waiting_time = ? WHERE pid = ?", (waiting_time, passenger_id))
    conn.commit()
...
# Reset and create passenger table
for passenger_count, output_file in zip(passenger_counts, output_files):
    ...
    for pid in range(passenger_count):
        if random.random() <= 0.7:
            ...
            arrival_stop = random.choice([stop for stop in stops if stop != 'B'])
    ...
    # Export to CSV with additional destination_time and distance columns
