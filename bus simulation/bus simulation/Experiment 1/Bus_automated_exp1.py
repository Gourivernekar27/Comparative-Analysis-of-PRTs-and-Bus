...
def save_to_csv(total_passenger_count, capacity):
    filename = f"exp1_passengerdata_{capacity}_{total_passenger_count}.csv"
    try:
...
def log_cost_value(total_distance_all_buses, total_passenger_count, capacity):
    ...
    # Check if the passenger count already exists in the table
    cursor.execute(f"SELECT number_of_passengers FROM {table_name} WHERE number_of_passengers = ?",
