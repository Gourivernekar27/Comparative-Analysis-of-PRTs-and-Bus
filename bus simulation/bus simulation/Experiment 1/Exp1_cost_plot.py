...
cursor.execute("SELECT number_of_passengers, cost_value FROM exp1_50_costdata WHERE number_of_passengers IN (392, 896, 2968, 4032, 5040) ORDER BY number_of_passengers")
...
cursor.execute("SELECT number_of_passengers, cost_value FROM exp1_80_costdata WHERE number_of_passengers IN (392, 896, 2968, 4032, 5040) ORDER BY number_of_passengers")
...
plt.plot(passengers1, cost_values1, marker='o', linestyle='-', label='exp1_50_costdata')
plt.plot(passengers2, cost_values2, marker='s', linestyle='-', label='exp1_80_costdata')
